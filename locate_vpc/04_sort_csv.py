...
def sort_csv_file(input_csv_path, sort_columns, output_csv_path):
    ...
    try:
        df = pd.read_csv(input_csv_path, dtype=str)  # 已經確保 CSV 欄位讀入為字串
...
def add_account_name(mapping_file, csv_file, output_file):
    ...
    try:
        ...
        # ------------------ 修改開始 ------------------
        # 將 mapping_df 的 Account 欄位轉成字串，確保 merge 時型別一致
        mapping_df["Account"] = mapping_df["Account"].astype(str)

        # 讀取要 enrich 的 CSV，也把 Account 欄位轉成字串
        data_df = pd.read_csv(csv_file, dtype=str)
        data_df["Account"] = data_df["Account"].astype(str)
        # ------------------ 修改結束 ------------------

        # ------------------ 新增檢查缺失帳號 ------------------
        missing_accounts = set(data_df["Account"]) - set(mapping_df["Account"])
        if missing_accounts:
            logging.warning(f"These accounts are missing in mapping file: {missing_accounts}")
            print(f"Warning: These accounts are missing in mapping file: {missing_accounts}")
        # ------------------ 檢查結束 ------------------
        ...
        # 重新排序欄位：AccountName, Account 放在最前面
